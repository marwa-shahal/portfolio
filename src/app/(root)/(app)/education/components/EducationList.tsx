import { IEducationItem } from '@/types/experience' // Adjust path as needed
...
const EducationList: React.FC<EducationListProps> = ({ data }) => {
  ...
  const renderItem = (item: IEducationItem) => (
    <div key={item.title} className="p-4 bg-white/30 border border-black/5 backdrop-blur dark:border-white/5 dark:bg-black/30">
      <div className="flex justify-between items-start mb-1">
        ...
        <p className="text-sm font-medium text-gray-500 dark:text-gray-400 flex-shrink-0 ml-4">{item.period}</p>
      </div>
      <p
        className={`text-sm italic font-medium mb-2 ${
          item.type === 'academic' ? 'text-indigo-600 dark:text-indigo-400' : 'text-teal-600 dark:text-teal-400'
        }`}
      >
  ...
  return (
    <div className="space-y-12">
      ...
      <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Academic Education</h2>
      <div className="space-y-6">{academic.map(renderItem)}</div>

      <hr className="border-gray-200 dark:border-gray-700" />

      {/* Section 2: Professional Certifications (Grid for density) */}
      <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Professional Certifications & Training</h2>
      {/* Uses a two-column layout on desktop for a dense, professional look */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{professional.map(renderItem)}</div>
    </div>
  )
}
